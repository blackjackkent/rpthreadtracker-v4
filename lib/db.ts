...
function parseConnectionString(connectionString: string): sql.config {
	...
	if (connectionString.includes("(localdb)")) {
		...
		const trustServerCertMatch = connectionString.match(
			/TrustServerCertificate=([^;]+)/i
		);
		...
		const config: sql.config = {
			...
			...(port && { port }),
			options: {
				encrypt: encryptMatch
					? encryptMatch[1].toLowerCase() === "true"
					: false,
				trustServerCertificate: trustServerCertMatch
					? trustServerCertMatch[1].toLowerCase() === "true"
					: false,
				enableArithAbort: true,
				...(instanceName && { instanceName }),
			},
...
export async function getUserByUsername(
	...
	const result = await db
		...
		.query(
			"SELECT * FROM dbo.AspNetUsers WHERE NormalizedUserName = @username"
		);
...
export async function getActiveThreadsCount(userId: string): Promise<number> {
	const db = await getDb();
	const result = await db.request().input("userId", sql.NVarChar, userId)
		.query(`
