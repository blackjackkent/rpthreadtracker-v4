    ...
    Credentials({
      ...
      async authorize(credentials) {
        try {
          ...
          const { getUserByEmail, getUserByUsername, updateUserPassword } = await import('./db');
          const { verifyPassword, hashPasswordBcrypt } = await import('./password-verifiers');
          ...
          const { verified, hashType } = await verifyPassword(password, user.PasswordHash);
